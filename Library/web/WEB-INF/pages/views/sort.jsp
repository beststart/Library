...
<div id="body">
    ...
    <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
        <c:forEach var="p" items="${pl}" varStatus="i">
            <div class="panel panel-default">
            <div class="panel-heading" role="tab" id="press_${p.id}">
                <h4 class="panel-title">
                    <a role="button" data-toggle="collapse" data-parent="#accordion" href="#list_${p.id}" aria-expanded="false" aria-controls="list_${p.id}">
                        ${p.name}
                    </a>
                </h4>
            </div>
            <div id="list_${p.id}" class="panel-collapse collapse" role="tabpanel" aria-labelledby="press_${p.id}">
                <div class="panel-body">
                    <div class="row">
                        <c:forEach var="b" items="${bl}">
                            <c:if test="${p.id==b.pressid}">
                                <div class="col-sm-6 col-md-4">
                                <div class="thumbnail">
                                    <img src="static/upload/${b.img}" alt="${b.name}" style="height: 175px;">
                                    <div class="caption">
                                        <h3>${b.name}</h3>
                                        <p>作者：${b.aname}</p>
                                        <p>出版社：${b.pname}</p>
                                        <p class="intro" title="${b.intro}">
                                            <c:choose>
                                                <c:when test="${fn:length(b.intro)>30}">
                                                    ${fn:substring(b.intro, 0,30 )}...
                                                </c:when>
                                                <c:otherwise>${b.intro}</c:otherwise>
                                            </c:choose>
                                        </p>
                                        <p>
                                            <a href="index?type=detail&id=${b.id}" class="btn btn-default" role="button" data-toggle="modal" data-target="#modal">详情</a>
                                            <a href="javascript:;" onclick="borBook(${b.id});" class="btn btn-default" role="button">借阅</a>
                                            <span>被借${b.count}次</span>
                                        </p>
                                    </div>
                                </div>
                            </div>
